...
function isValidOverrideFileName(fileName) {
  ...
  if (fileName.toLowerCase() === WEBAPP_OVERRIDE_LOADER_NAME.toLowerCase()) {
    return false;
  }
...
function listWebappOverrideFiles() {
  ensureDirectoryExists(WEBAPP_CFG_DIR);
  return fs.readdirSync(WEBAPP_CFG_DIR)
    .filter((name) => isValidOverrideFileName(name) && fs.statSync(path.join(WEBAPP_CFG_DIR, name)).isFile())
    .sort((a, b) => a.localeCompare(b));
}

function ensureServerCfgExecsOverrideLoader() {
  ensureDirectoryExists(CFG_DIR);

  let content = '';
  if (fs.existsSync(SERVER_CFG_PATH)) {
    content = fs.readFileSync(SERVER_CFG_PATH, 'utf8');
  }

  const lines = content.split(/\r?\n/);
  const hasExec = lines.some((line) => {
    const code = line.split('//')[0].trim();
    return /^exec\s+"?webapp\/overrides_loader\.cfg"?$/i.test(code);
  });
  ...
  const nextLines = lines.filter((line, idx) => !(idx === lines.length - 1 && line === ''));
  nextLines.push('', '// Webapp override loader (auto-managed)', WEBAPP_OVERRIDE_LOADER_EXEC);
  fs.writeFileSync(SERVER_CFG_PATH, nextLines.join('\n') + '\n', 'utf8');
  return true;
}

function syncWebappOverrideLoader() {
  ensureDirectoryExists(WEBAPP_CFG_DIR);
  const overrideFiles = listWebappOverrideFiles();

  const loaderLines = [
    '// Auto-generated by l4d2-webapp. Do not edit this file manually.',
    '// Put your override files (*.cfg) in cfg/webapp/ to have them auto-executed.',
    ''
  ];

  overrideFiles.forEach((fileName) => {
    loaderLines.push(`exec webapp/${fileName}`);
  });

  loaderLines.push('');
  const nextLoaderContent = loaderLines.join('\n');

  let loaderUpdated = false;
  let currentLoaderContent = '';
  if (fs.existsSync(WEBAPP_OVERRIDE_LOADER_PATH)) {
    currentLoaderContent = fs.readFileSync(WEBAPP_OVERRIDE_LOADER_PATH, 'utf8');
  }

  if (currentLoaderContent !== nextLoaderContent) {
    if (fs.existsSync(WEBAPP_OVERRIDE_LOADER_PATH)) {
      try {
        fs.copyFileSync(WEBAPP_OVERRIDE_LOADER_PATH, `${WEBAPP_OVERRIDE_LOADER_PATH}.bak`);
      } catch (backupErr) {}
    }
    fs.writeFileSync(WEBAPP_OVERRIDE_LOADER_PATH, nextLoaderContent, 'utf8');
    loaderUpdated = true;
  }

  const serverCfgUpdated = ensureServerCfgExecsOverrideLoader();
  return {
    overrideFiles,
    loaderUpdated,
    serverCfgUpdated,
...
app.get('/api/webapp/overrides/files', (req, res) => {
  try {
    const syncInfo = syncWebappOverrideLoader();
    res.json({
      files: syncInfo.overrideFiles,
      loader: {
        path: 'cfg/webapp/overrides_loader.cfg',
        exec: syncInfo.loaderExec,
        updated: syncInfo.loaderUpdated
      },
      serverCfgUpdated: syncInfo.serverCfgUpdated
    });
  } catch (e) {
    res.status(500).json({ error: e.message, files: [] });
  }
...
app.get('/api/webapp/overrides/read', (req, res) => {
  const file = String(req.query.file || '');
  if (!isValidOverrideFileName(file)) {
    return res.status(400).json({ error: 'Invalid override file name' });
  }

  const filePath = path.join(WEBAPP_CFG_DIR, file);
...
app.post('/api/webapp/overrides/write', (req, res) => {
  const { file, content, applyNow } = req.body || {};
  if (!isValidOverrideFileName(file) || typeof content !== 'string') {
    return res.status(400).json({ error: 'Invalid parameters' });
  }
  ...
  try {
    ...
    if (applyNow) {
      sendToGame(WEBAPP_OVERRIDE_LOADER_EXEC, () => {});
    }

    res.json({ success: true, file, overrideSync: syncInfo, applied: !!applyNow });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.post('/api/webapp/overrides/sync', (req, res) => {
  const applyNow = !!(req.body && req.body.applyNow);
  try {
    const syncInfo = syncWebappOverrideLoader();
    if (applyNow) {
      sendToGame(WEBAPP_OVERRIDE_LOADER_EXEC, () => {});
    }
    res.json({ success: true, overrideSync: syncInfo, applied: applyNow });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/dev/upload', (req, res) => {
  ...
  try {
    ...
    let overrideSync = null;
    if (
      rootKey === 'game' &&
      relativePath.toLowerCase().startsWith('left4dead2/cfg/webapp/') &&
      relativePath.toLowerCase().endsWith('.cfg')
    ) {
      overrideSync = syncWebappOverrideLoader();
    }
